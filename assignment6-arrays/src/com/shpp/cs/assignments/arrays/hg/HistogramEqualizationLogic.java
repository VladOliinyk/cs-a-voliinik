...
public class HistogramEqualizationLogic {
    ...
    public static int[] cumulativeSumFor(int[] histogram) {
        ...
        cumulativeFrequency[0] = histogram[0];
        for (int i = 1; i < histogram.length; i++) {
            cumulativeFrequency[i] = cumulativeFrequency[i - 1] + histogram[i];
        }
    ...
    public static int[][] equalize(int[][] luminances) {
        ...
        for (int i = 0; i < luminances.length; i++) {
            for (int j = 0; j < luminances[0].length; j++) {
                int index = luminances[i][j];
                luminances[i][j] = MAX_LUMINANCE * cumulativeHistogram[index] / totalPixels;
            }
        }
        return luminances;
    }
